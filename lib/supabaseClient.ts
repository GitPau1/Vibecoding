...
// To work around this, we previously used `any` for jsonb columns. However, to provide better type safety and resolve
// the compilation errors, we are now using specific types for these columns (`Player[]` and `string[]`).
// This breaks the recursion and allows TypeScript to correctly infer the types for other columns.

// Player interface defined locally to break potential type-dependency cycles
interface Player {
    id: number;
    name: string;
    team: string;
...
-- 5. Set up trigger to automatically create a profile for new users from metadata
...
-- 6. Update RLS policies to require authentication for content creation
-- This makes the app more secure by ensuring only logged-in users can create content.
CREATE POLICY "Allow authenticated users to insert votes" ON votes FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Allow authenticated users to insert articles" ON articles FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Allow authenticated users to insert x_posts" ON x_posts FOR INSERT TO authenticated WITH CHECK (true);

-- Update squad_players policies
...
CREATE POLICY "Allow authenticated users to manage squad_players" ON squad_players FOR (INSERT, UPDATE, DELETE) TO authenticated WITH CHECK (true);
