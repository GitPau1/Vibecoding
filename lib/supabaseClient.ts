...
// Define a non-recursive type for players to avoid deep type instantiation errors.
interface DbPlayer {
  id: number;
  name: string;
  team: string;
...
// To work around this, we are now using specific types for the jsonb columns (`players` and `comments`).
...
BEGIN
    ...
    LOOP
        ...
        -- Append comment if it's not null or empty
        IF comment_in IS NOT NULL AND comment_in != '' THEN
            UPDATE public.vote_options
            SET comments = comments || jsonb_build_array(comment_in)
            WHERE id = option_id_in;
        END IF;
    END LOOP;
END;
